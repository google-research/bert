...
class FullTokenizer(object):
    """Runs end-to-end tokenziation. 端到端分词"""
...
class BasicTokenizer(object):
    ...
    根据空格、标点进行普通分词，返回词的列表。对中文，返回字列表"""
    ...
    def _run_split_on_punc(self, text):
        """Splits punctuation(标点) on a piece of text."""  # 用标点切分
        chars = list(text)
        i = 0
        start_new_word = True
        output = []
    ...
    def _tokenize_chinese_chars(self, text):
        """Adds whitespace around any CJK(中日韩) character.
        按字切分中文，通过在字两侧添加空格实现"""
        output = []
        for char in text:
            cp = ord(char)
...
class WordpieceTokenizer(object):
    ...
    将BasicTokenizer结果进行细粒度的切分，主要是去除未登录词对模型效果的影响。"""
