...
flags.DEFINE_string("input_file", './sample_text1.txt',
...
flags.DEFINE_integer("max_predictions_per_seq", 20,
                     "Maximum number of masked LM predictions per sequence.")  # 一个句子中最多[MASK]标记

flags.DEFINE_integer("random_seed", 12345, "Random seed for data generation.")

flags.DEFINE_integer(
    "dupe_factor", 10,
    "Number of times to duplicate the input data (with different masks).")  # 重复参数

flags.DEFINE_float("masked_lm_prob", 0.15, "Masked LM probability.")  # token被mask的比例
...
class TrainingInstance(object):
    """A single training instance (sentence pair).  # 训练样本的类"""
...
def create_instances_from_document(
    ...
    while i < len(document):
        segment = document[i]  # 句子
        current_chunk.append(segment)
        current_length += len(segment)
        # 句子依次加入到current_chunk中，直到全部加完或者达到最大长度
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                tokens.append("[SEP]")
                segment_ids.append(1)
                # 调用create_masked_lm_predictions来随机对Token进行mask
...
def main(_):
    ...
    for input_pattern in FLAGS.input_file.split(","):
        input_files.extend(tf.gfile.Glob(input_pattern))  # './sample_text1.txt'
...
if __name__ == "__main__":
    flags.mark_flag_as_required("input_file")  # 参数'input_file'，必须输入
