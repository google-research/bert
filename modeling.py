...
def embedding_lookup(input_ids,
  ...
  Args:
    input_ids: int32 Tensor of shape [batch_size, seq_length] containing word ids.
        # 输入为2维, [[31, 51, 99], [15, 5, 0]], shape = [2, 3]
  ...
  if input_ids.shape.ndims == 2:
    input_ids = tf.expand_dims(input_ids, axis=[-1])  # [2, 3, 1]
  ...
  if use_one_hot_embeddings:
    ...
    output = tf.gather(embedding_table, flat_input_ids)  # (6, 128)

  input_shape = get_shape_list(input_ids)   # (2, 3, 1)
...
def embedding_postprocessor(input_tensor,
  ...
  Args:
    ...
    max_position_embeddings: int. Maximum sequence length that might ever be used with this model.
        This can be longer than the sequence length of input_tensor, but cannot be shorter.
  ...
  """
  input_shape = get_shape_list(input_tensor, expected_rank=3)  # (2, 3, 128)
  batch_size = input_shape[0]
  seq_length = input_shape[1]
  width = input_shape[2]
  ...
  if use_token_type:
    ...
    token_type_table = tf.get_variable(
        ...
        initializer=create_initializer(initializer_range))  # (16, 128)
    ...
    token_type_embeddings = tf.matmul(one_hot_ids, token_type_table)  # (6, 128)
  ...
  if use_position_embeddings:
    ...
    with tf.control_dependencies([assert_op]):
      full_position_embeddings = tf.get_variable(
          ...
          initializer=create_initializer(initializer_range))  # (512, 128)
      ...
      # the batch size.
      position_broadcast_shape = []  # (1, 3, 128)
      for _ in range(num_dims - 2):
        position_broadcast_shape.append(1)
      position_broadcast_shape.extend([seq_length, width])
      position_embeddings = tf.reshape(position_embeddings,
                                       position_broadcast_shape)  # (3, 128)
      output += position_embeddings

  output = layer_norm_and_dropout(output, dropout_prob)  # (2, 3, 128)
  return output
...
if __name__ == '__main__':
    ...
    #
    # # input_shape = get_shape_list(input_ids)  # 883
    #
    #
    # with tf.Session() as sess:
    #     tf.global_variables_initializer().run()
    #     # print(sess.run(input_shape, feed_dict={chars_input: [[31, 51, 99], [15, 5, 0]]}))
    #     print(sess.run(output, feed_dict={chars_input: [[8, 5, 0]]}))
